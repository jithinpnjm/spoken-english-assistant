import { professionalEnglishContent, professionalEnglishModuleIds } from "../src/server/professionalEnglishContent";
import { getCurriculumModule } from "../src/server/curriculumRegistry";
import { getTeachingContent, isHandAuthoredContent } from "../src/server/curriculumContentRegistry";

function fail(message: string): never {
  console.error(`[professional-content] FAIL: ${message}`);
  process.exit(1);
}

if (!professionalEnglishModuleIds.length) fail("No Professional English module IDs configured");
if (!professionalEnglishContent.length) fail("No Professional English content generated");

const expectedIds: string[] = [];
for (const moduleId of professionalEnglishModuleIds) {
  const module = getCurriculumModule(moduleId);
  if (!module) fail(`Missing curriculum module ${moduleId}`);
  expectedIds.push(...module.subsections.map((item) => item.id));
}

const contentIds = professionalEnglishContent.map((item) => item.subsectionId);
for (const id of expectedIds) {
  if (!contentIds.includes(id)) fail(`Missing Professional English content for ${id}`);
  if (!isHandAuthoredContent(id)) fail(`${id} is not marked as hand-authored/common authored content`);
  const content = getTeachingContent(id);
  if (!content.ruleSummary.trim()) fail(`${id} has empty ruleSummary`);
  if (!content.explanation.Intermediate.includes("Goal:")) fail(`${id} intermediate explanation must include Goal`);
  if (!content.explanation.Advanced.includes("Core pattern:")) fail(`${id} advanced explanation must include Core pattern`);
  if (content.examples.length < 5) fail(`${id} should have at least five examples`);
  if (content.commonMistakes.length < 3) fail(`${id} should have at least three common mistakes`);
  if (content.activityTemplates.drill.length < 4) fail(`${id} should have at least four drills`);
  if (content.successCriteria.length < 4) fail(`${id} should have at least four success criteria`);
}

for (const id of contentIds) {
  if (!expectedIds.includes(id)) fail(`Unknown Professional English content subsection ${id}`);
}

console.log(`[professional-content] OK: ${professionalEnglishContent.length} common authored lessons across ${professionalEnglishModuleIds.length} Professional English modules.`);
