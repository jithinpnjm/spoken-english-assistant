...
export default function CurriculumProgressPanel({ courses, cursor, selectedLevel, selectedModuleId, onSelectedModuleChange, onStartLevel, onStartModule, selectedTrackTitle, allowedModuleIds, isBusy }: CurriculumProgressPanelProps) {
    ...
    <section className="mt-5 rounded-2xl border border-cyan-400/20 bg-cyan-500/10 p-4 text-sm">
      <div className="flex items-center justify-between gap-3 mb-3">
        <div>
          ...
          <p className="text-[11px] text-slate-400">{selectedTrackTitle ? `Track: ${selectedTrackTitle}` : `Showing ${selectedLevel} modules`}</p>
        </div>
      ...
      <div className="space-y-2 mb-4">
        ...
        <div className="rounded-xl bg-black/20 border border-white/10 p-3">
          <p className="text-[10px] uppercase tracking-wider text-slate-400">Lesson</p>
      ...
      <div className="space-y-2">
        <div className="flex gap-2">
          <input value={searchDraft} onChange={(e) => setSearchDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") setSearchTerm(searchDraft); }} placeholder="Search module/lesson..." className="min-w-0 flex-1 bg-slate-900 border border-white/10 rounded-xl px-3 py-2 text-xs outline-none" />
        ...
        <p className="text-[10px] text-slate-400">{filteredModules.length} module(s) visible{searchTerm ? ` matching “${searchTerm}”` : ""}.</p>
