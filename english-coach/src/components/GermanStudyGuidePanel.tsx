...
7. Give the learner the spaced revision plan so they review the lesson later.
8. Only then ask the learner to build one short sentence.
        ...
        <div className="space-y-4">
          <div className="rounded-3xl border border-amber-400/20 bg-amber-500/10 p-5">
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div><p className="text-xs uppercase tracking-widest text-amber-300">Lesson {selected.lessonNo} of 65 · A1</p><h2 className="mt-1 text-2xl font-bold text-slate-100">{selected.titleEn}</h2><p className="text-sm text-slate-400 mt-0.5">{selected.titleDe}</p></div>
              <button onClick={isLiveActive ? onStopLive : () => onPracticeWithSky(buildLessonContext(selected, learnerName))} className={`shrink-0 flex items-center gap-2 rounded-2xl px-5 py-3 text-sm font-bold transition-colors ${isLiveActive ? "bg-red-600 hover:bg-red-500 text-white" : "bg-amber-500 hover:bg-amber-400 text-slate-950"}`}>{isLiveActive ? <><StopCircle className="h-4 w-4" /> Stop</> : <><Mic className="h-4 w-4" /> Practice with Sky</>}</button>
            </div>
          </div>

          <div className="rounded-2xl border border-indigo-400/20 bg-indigo-500/10 p-4"><p className="text-xs uppercase tracking-widest text-indigo-200 mb-3">Teacher sentence-building path</p><div className="grid gap-3 md:grid-cols-4"><div className="rounded-xl border border-white/10 bg-black/20 p-3"><p className="text-xs uppercase tracking-widest text-slate-400">1. Meaning</p><p className="mt-1 text-sm text-slate-100">What do I want to say?</p></div><div className="rounded-xl border border-white/10 bg-black/20 p-3"><p className="text-xs uppercase tracking-widest text-slate-400">2. Person + verb</p><p className="mt-1 text-sm text-slate-100">ich + trinken → ich trinke</p></div><div className="rounded-xl border border-white/10 bg-black/20 p-3"><p className="text-xs uppercase tracking-widest text-slate-400">3. Article/case</p><p className="mt-1 text-sm text-slate-100">ein Termin → einen Termin</p></div><div className="rounded-xl border border-white/10 bg-black/20 p-3"><p className="text-xs uppercase tracking-widest text-slate-400">4. Full sentence</p><p className="mt-1 text-sm text-slate-100">Ich habe einen Termin.</p></div></div><p className="mt-3 text-sm leading-relaxed text-slate-300">This is the production path: choose the person, transform the verb, choose the article/case, then add object, time, place, or reason.</p></div>
