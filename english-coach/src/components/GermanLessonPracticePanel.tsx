...
export default function GermanLessonPracticePanel({ lesson }: GermanLessonPracticePanelProps) {
  return (
    <div className="rounded-2xl border border-lime-400/20 bg-lime-500/10 p-4">
      ...
      <p className="mt-2 text-sm leading-relaxed text-slate-300">Complete controlled drills before free speaking with Sky.</p>
      ...
      {lesson.skyPracticePrompts.length > 0 && (
        <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4">
          <p className="text-xs uppercase tracking-widest text-lime-200 mb-3">Speak with Sky after the drills</p>
          <div className="grid gap-2 md:grid-cols-2">
            {lesson.skyPracticePrompts.slice(0, 5).map((prompt, index) => (
              <div key={`${prompt}-${index}`} className="rounded-xl border border-lime-400/10 bg-lime-500/10 p-3 text-sm leading-relaxed text-slate-100">{prompt}</div>
            ))}
          </div>
        </div>
      )}
